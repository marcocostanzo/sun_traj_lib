...
protected:
  ...
  */
  ...
  /*
      Trajectory for the scalar s variable should be a traj from 0 to
     final_angle
      s = 0 -> _pi   &   s = 1 -> _pf
  */
...
public:
  ...
  */
  ...
  /*
     ...
     solution
  */
  static void two_points_2_center(const TooN::Vector<3> &r_hat,
                                  const TooN::Vector<3> &pi,
                                  const TooN::Vector<3> &pf, double rho,
                                  TooN::Vector<3> &c, double &angle,
                                  bool sign_plus = true);
...
using Position_Circumference_Traj_Ptr =
    std::unique_ptr<Position_Circumference_Traj>;
}
